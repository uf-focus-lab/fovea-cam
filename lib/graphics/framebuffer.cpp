...
void FrameBuffer::buffer_init() {
  ...
  buf_bytes = vinfo.xres_virtual * finfo.line_length;
  buf_pixels = buf_bytes / bytes_per_pixel;
  /* Map memory accordingly */
  // framebuffer = (uint8_t *)mmap(NULL, buf_bytes, PROT_READ | PROT_WRITE,
  //                               MAP_SHARED, fd, 0);
  // if (framebuffer == MAP_FAILED) {
  //   throw std::runtime_error(
  //       "[framebuffer] Failed to map framebuffer device to memory.");
  // }
}

void FrameBuffer::buffer_free() {
  // blank screen
  // memset(framebuffer, 0, buf_bytes);
  // release mapped memory
  if (framebuffer != nullptr)
    munmap((void *)framebuffer, buf_bytes);
}
...
FrameBuffer::FrameBuffer(std::string path) {
  ...
  std::cerr << "xres           " << vinfo.xres << std::endl
            ...
            << "buffer size    " << buf_bytes << std::endl;
#endif
}
...
info FrameBuffer::info() {
  return {.width = vinfo.xres_virtual,
          ...
          .line_length = finfo.line_length / bytes_per_pixel,
...
void FrameBuffer::from(uint8_t *src) { from(src, 0, buf_bytes); }
